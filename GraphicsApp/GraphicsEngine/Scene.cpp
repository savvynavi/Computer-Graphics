...
void Scene::Draw(int windowWidth, int windowHeight){
	...
	for each(Instance* object in m_instances){
		object->Draw(m_projectionMatrix, m_viewMatrix, m_light.ambient, m_light.diffuse, m_light.specular, m_light.direction, m_numLights, &m_pointLightPos[0], &m_pointLightColour[0], &m_intensity[0]);
	}
}
